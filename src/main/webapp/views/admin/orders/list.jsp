			...
			<div class="main-content-inner">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Danh sách tất cả đơn hàng</li>
						</ul><!-- /.breadcrumb -->
										...
										<tbody>
											<c:forEach var="item" items="${list_orders}">
												<tr>
													...
													<td><c:choose>
															<c:when test="${item.status eq 'Đã nhận được hàng'}">
									...
									<div class="clearfix">
										...
										<ul class="pagination">
											<li class="page-item"><c:choose>
													<c:when test="${index > 0}">
														<a
															href="<c:url value='/admin-order?index=${index-1}'></c:url>">Previous</a>
													</c:when>
													<c:otherwise>
											...
											<c:forEach begin="0" end="${empty list_orders ? 0 :sizepage-1}" var="item">
												<li class="page-item"><c:choose>
														<c:when test="${item == index}">
															<a
																href="<c:url value='/admin-order?index=${item}'></c:url>"
											...
											<li class="page-item"><c:choose>
													<c:when test="${index < sizepage-1}">
														<a
															href="<c:url value='/admin-order?index=${index+1}'></c:url>"
